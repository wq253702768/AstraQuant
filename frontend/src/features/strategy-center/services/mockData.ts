...
export const strategyVersions = versions;
export const mockVersions = versions;
export const mockStrategies = strategies;
export const mockVersionHistory = versions.map((version) => ({
  ...version,
  status: version.isCurrent ? '当前版本' : version.status === 'backtested' ? '已回测' : '已归档',
}));

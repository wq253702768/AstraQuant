...
class RiskClient:
    def __init__(self, base_url: str = settings.risk_engine_url): self.base_url=base_url.rstrip("/")
    async def request(self, method: str, path: str, request: Request, json: dict[str, Any] | None=None, params: dict[str, Any] | None=None):
        try:
            ...
            return JSONResponse(status_code=502, content=error_response("RISK_ENGINE_UNAVAILABLE","Risk Engine 暂不可用",request.state.trace_id).model_dump())

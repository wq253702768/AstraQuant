...
from app.application.services.copy_strategy_version_service import CopyStrategyVersionService
...
from app.application.services.publish_strategy_version_service import PublishStrategyVersionService
...
from app.schemas.strategy_version import CopyStrategyVersionRequest, CreateStrategyVersionRequest, PublishStrategyVersionRequest
...
@router.post("/{strategy_id}/versions/{version_id}/publish")
async def publish_version(strategy_id: str, version_id: str, payload: PublishStrategyVersionRequest, request: Request, session: AsyncSession = Depends(get_db_session), operator_id: str = Depends(get_operator_id)):
    result = await PublishStrategyVersionService(session).execute(strategy_id, version_id, payload.publish_note, operator_id, getattr(request.state, "trace_id", None))
    await session.commit()
    return success_response(result, request)

@router.post("/{strategy_id}/versions/{version_id}/copy")
async def copy_version(strategy_id: str, version_id: str, payload: CopyStrategyVersionRequest, request: Request, session: AsyncSession = Depends(get_db_session), operator_id: str = Depends(get_operator_id)):
    result = await CopyStrategyVersionService(session).execute(strategy_id, version_id, payload.change_reason, operator_id)
    await session.commit()
    return success_response(result, request)

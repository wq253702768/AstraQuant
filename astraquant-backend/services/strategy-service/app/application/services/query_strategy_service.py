...
class QueryStrategyService:
    ...
    async def list_templates(self) -> dict:
        items = [StrategyTemplateListItem(id=item.id, code=item.code, name=item.name, strategy_type=item.strategy_type, description=item.description) for item in await self.template_repo.list_enabled()]
        return {"items": items}
    ...
    async def get_template(self, template_id: str) -> StrategyTemplateDetail:
        ...
        return StrategyTemplateDetail(id=item.id, code=item.code, name=item.name, strategy_type=item.strategy_type, description=item.description, default_params=item.default_params, param_schema=item.param_schema, risk_schema=item.risk_schema)
    ...
    async def list_strategies(self, status: str | None, strategy_type: str | None, keyword: str | None, page: int, page_size: int) -> dict:
        ...
        for strategy in strategies:
            ...
            items.append(StrategyListItem(id=strategy.id, name=strategy.name, code=strategy.code, strategy_type=strategy.strategy_type, status=strategy.status, latest_version=latest.version if latest else None, created_at=strategy.created_at))
    ...
    async def get_strategy(self, strategy_id: str) -> StrategyDetail:
        ...
        return StrategyDetail(id=strategy.id, name=strategy.name, code=strategy.code, strategy_type=strategy.strategy_type, status=strategy.status, description=strategy.description, versions=[StrategyVersionSummary(id=item.id, version=item.version, status=item.status, params_hash=item.params_hash, created_at=item.created_at) for item in versions])

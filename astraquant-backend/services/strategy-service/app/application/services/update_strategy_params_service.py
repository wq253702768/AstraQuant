...
class UpdateStrategyParamsService:
    ...
    async def execute(self, version_id: str, payload: UpdateStrategyParamsRequest, operator_id: str, trace_id: str | None) -> UpdateStrategyParamsResponse:
        ...
        await self.log_repo.create(StrategyStatusLogModel(strategy_id=strategy.id, strategy_version_id=version.id, from_status=version.status, to_status=version.status, reason="参数校验通过", operator_id=operator_id))
        now = datetime.now(UTC).isoformat()
        await self.publisher.publish(topics.STRATEGY_STATUS_CHANGED, {"event_type": topics.STRATEGY_STATUS_CHANGED, "strategy_id": strategy.id, "strategy_version_id": version.id, "from_status": version.status, "to_status": version.status, "reason": "参数校验通过", "operator_id": operator_id, "created_at": now})

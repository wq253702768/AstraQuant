...
COMPOSE="docker compose --env-file '$ASTRA_STAGING_ENV_FILE' -f deploy/staging/docker-compose.sprint3-part1.yml"

echo "==> Building and starting Sprint 3 Part 1 stack"
ssh "${SSH_OPTS[@]}" "$REMOTE" "cd '$ASTRA_STAGING_APP_DIR/astraquant-backend' && $COMPOSE up -d --build"

echo "==> Running auth-service migrations and seed"

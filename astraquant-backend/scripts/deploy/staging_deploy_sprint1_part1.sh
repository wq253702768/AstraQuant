...
COMPOSE_FILE="deploy/staging/docker-compose.sprint1-part1.yml"
COMPOSE="docker compose --env-file '$ASTRA_STAGING_ENV_FILE' -f '$COMPOSE_FILE'"
...
ssh "${SSH_OPTS[@]}" "$REMOTE" "test -f '$ASTRA_STAGING_ENV_FILE' || (echo 'Missing $ASTRA_STAGING_ENV_FILE on server.' >&2; exit 2)"

echo "==> Building and starting Sprint 1 Part 1 stack"
...
ssh "${SSH_OPTS[@]}" "$REMOTE" "cd '$ASTRA_STAGING_APP_DIR/astraquant-backend' && $COMPOSE run --rm -v '$ASTRA_STAGING_APP_DIR/astraquant-backend/scripts:/scripts:ro' auth-service python /scripts/create_admin_user.py"

echo "==> Sprint 1 Part 1 deployment finished"

...
COMPOSE="docker compose --env-file '$ASTRA_STAGING_ENV_FILE' -f deploy/staging/docker-compose.sprint4-part1.yml"

echo "==> Building and starting Sprint 4 Part 1 stack"
...
ssh "${SSH_OPTS[@]}" "$REMOTE" "cd '$ASTRA_STAGING_APP_DIR/astraquant-backend' && $COMPOSE run --rm market-data-service alembic upgrade head"
...
echo "==> Sprint 4 Part 1 deployment finished"
